    ...
    def get_full_data(self, request):
        ...
        if 'toothbrush_type' in request.query_params:
            ...
            delivery_statuses = FullOrder.objects.filter(
                toothbrush_type__iexact=toothbrush_type).aggregate(
                delivery_successful=Count(
                    'pk', filter=Q(delivery_status='Delivered')),
                delivery_unsuccessful=Count(
                    'pk', filter=Q(delivery_status='Unsuccessful')),
                delivery_in_transit=Count(
                    'pk', filter=Q(delivery_status='In Transit'))
            )
            ...
            data_by_postcode_serializer = FullPostcodeDataSerializer(
                data_by_postcode, many=True)
            tb_sales_by_age_serializer = TBSalesByAgeSerializer(
                tb_sales_by_age, many=True)
            total_orders_serializer = TotalOrdersSerializer(
                total_orders, many=False)
            delivery_status_serializer = DeliveryStatusSerializer(
                delivery_statuses, many=False)
            delivery_delta_serializer = DeliveryDeltaSerializer(
                avg_delivery_delta, many=False)
            customer_age_serializer = CustomerAgeSerializer(
                customer_age, many=False)
        ...
        sales_by_age_serializer = TBSalesByAgeSerializer(
            tb_sales_by_age, many=True)
        data_by_postcode_serializer = FullPostcodeDataSerializer(
            data_by_postcode, many=True)
        customer_age_serializer = CustomerAgeSerializer(customer_age)
        avg_delivery_delta_serializer = DeliveryDeltaSerializer(
            avg_delivery_delta)
        tb_sales_by_age_serializer = TBSalesByAgeSerializer(
            tb_sales_by_age, many=True)
        tb_2000_order_quantity_serializer = OrderQuantitySerializer(
            tb_2000_order_quantity, many=True)
        tb_4000_order_quantity_serializer = OrderQuantitySerializer(
            tb_4000_order_quantity, many=True)
        tb_2000_order_quantity_by_postcode_serializer = OrderQuantitySerializer(
            tb_2000_order_quantity_by_postcode, many=True)
        tb_4000_order_quantity_by_postcode_serializer = OrderQuantitySerializer(
            tb_4000_order_quantity_by_postcode, many=True)
        total_orders_serializer = TotalOrdersSerializer(
            total_orders, many=False)
        delivery_status_serializer = DeliveryStatusSerializer(
            delivery_statuses, many=False)
...
class NullOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if 'count_null_orders' in self.request.query_params:
            ...
            queryset = NullOrder.objects.aggregate(
